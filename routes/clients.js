...
router.get('/:id', async (req, res) => {
  try {
    ...
    const [client, compras, cobros, eventos, tareas, entregados, turnos] = await Promise.all([
      ...
      db.query(`
        SELECT a.*, p.model AS producto_model, p.storage_gb AS producto_gb
        FROM appointments a
        LEFT JOIN productos p ON p.id = a.producto_id
        WHERE a.contact_id = (SELECT contact_id FROM clients WHERE id=$1)
...
router.post('/from-contact/:contactId', async (req, res) => {
  try {
    ...
    // Al convertir: se pasan TODOS los datos del contacto (teléfono, instagram, ciudad, notas)
    // y se desactivan IA y seguimientos por defecto
    const r = await db.query(`
      INSERT INTO clients (contact_id, name, last_name, phone, instagram, city, notes,
        ai_enabled, followups_enabled, ai_disabled_at, ai_disabled_reason)
      VALUES ($1,$2,$3,$4,$5,$6,$7,FALSE,FALSE,NOW(),'Convertido a cliente')
      RETURNING *
    `, [contactId, parts[0] || '', parts.slice(1).join(' ') || '', c.phone, c.instagram, c.city, c.notes]);
