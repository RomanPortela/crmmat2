...
// GET /api/proveedores/pendientes — items encargados a proveedores que
// todavía no se cargaron a stock, pensado para que el agente de IA los
// ofrezca como última instancia cuando algo no está disponible ahora.
router.get('/pendientes', async (req, res) => {
  try {
    const r = await db.query(`
      SELECT soi.id, soi.model, soi.storage_gb, soi.color, soi.battery_pct,
             soi.retail_price, soi.quantity, soi.condition_notes,
             so.status AS order_status, so.estimated_arrival,
             s.name AS supplier_name
      FROM supplier_order_items soi
      JOIN supplier_orders so ON so.id = soi.order_id
      LEFT JOIN suppliers s ON s.id = so.supplier_id
      WHERE soi.loaded_to_stock = FALSE
        AND so.status IN ('pendiente', 'en_camino')
      ORDER BY so.estimated_arrival ASC NULLS LAST
    `);
    res.json(r.rows);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
