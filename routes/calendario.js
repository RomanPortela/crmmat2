    ...
    if (Array.isArray(f.products)) {
      for (const p of f.products) {
        ...
        if (p.reserved === true) {
          ...
          // Crear el cobro de seña si el evento tiene monto de seña y no existe ya uno para este evento
          if (f.sena_amount) {
            const existingCobro = await client.query(
              'SELECT id FROM cobros WHERE calendar_event_id=$1', [eventId]
            );
            if (!existingCobro.rows[0]) {
              let clientId = null;
              if (f.client_id) clientId = f.client_id;
              else if (f.contact_id) {
                const cl = await client.query('SELECT id FROM clients WHERE contact_id=$1', [f.contact_id]);
              ...
              const cobro = await client.query(`
                INSERT INTO cobros (client_id, producto_id, type, total_amount, notes, receipt_num, calendar_event_id)
                VALUES ($1,$2,'seña',$3,$4,$5,$6) RETURNING id
              `, [clientId, p.producto_id, f.sena_amount, `Seña por evento: ${f.title}`, receiptNum, eventId]);

              await client.query(`
                INSERT INTO cobro_payments (cobro_id, method, amount) VALUES ($1,'efectivo_pesos',$2)
              `, [cobro.rows[0].id, f.sena_amount]);
            }
          }
        }
      }
...
router.patch('/:id/producto/:productoId', async (req, res) => {
  ...
  try {
    ...
    if (reserved) {
      ...
      // Crear cobro de seña si el evento tiene monto definido y no existe ya uno
      const ev = await client.query('SELECT * FROM calendar_events WHERE id=$1', [req.params.id]);
      const senaAmount = ev.rows[0]?.["seña_amount"];
      if (senaAmount) {
        const existingCobro = await client.query('SELECT id FROM cobros WHERE calendar_event_id=$1', [req.params.id]);
        if (!existingCobro.rows[0]) {
          let clientId = null;
          if (ev.rows[0].client_id) clientId = ev.rows[0].client_id;
          else if (ev.rows[0].contact_id) {
            const cl = await client.query('SELECT id FROM clients WHERE contact_id=$1', [ev.rows[0].contact_id]);
            if (cl.rows[0]) clientId = cl.rows[0].id;
          }
          const num = await client.query(`SELECT COUNT(*)+1 AS n FROM cobros`);
          const receiptNum = `R-${String(num.rows[0].n).padStart(5, '0')}`;
          const cobro = await client.query(`
            INSERT INTO cobros (client_id, producto_id, type, total_amount, notes, receipt_num, calendar_event_id)
            VALUES ($1,$2,'seña',$3,$4,$5,$6) RETURNING id
          `, [clientId, req.params.productoId, senaAmount, `Seña por evento: ${ev.rows[0].title}`, receiptNum, req.params.id]);
          await client.query(`
            INSERT INTO cobro_payments (cobro_id, method, amount) VALUES ($1,'efectivo_pesos',$2)
