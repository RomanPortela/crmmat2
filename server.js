  ...
  for (const t of TEMPLATES) {
    await db.query(`
      ...
      ON CONFLICT (section, title, item_order) DO UPDATE SET
        ...
        message = CASE
          WHEN message_templates.message = message_templates.default_message
          THEN EXCLUDED.message ELSE message_templates.message END,
...
app.post('/webhook/lead', async (req, res) => {
  ...
  try {
    ...
    // El teléfono ya no es único en la base, pero acá sí conviene identificar
    // al mismo remitente de WhatsApp para no crear una conversación nueva
    // por cada mensaje. Se busca primero por whatsapp_id, y si no, por phone.
    let contact = null;
    if (whatsapp_id) {
      const byWa = await db.query('SELECT * FROM contacts WHERE whatsapp_id = $1', [whatsapp_id]);
      contact = byWa.rows[0] || null;
    }
    if (!contact && phone) {
      const byPhone = await db.query('SELECT * FROM contacts WHERE phone = $1 ORDER BY created_at ASC LIMIT 1', [phone]);
      contact = byPhone.rows[0] || null;
    }
