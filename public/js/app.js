...
document.getElementById('prod-search')?.addEventListener('input', e => { prodSearch = e.target.value; prodPage = 0; loadProductos(); });
document.getElementById('prod-filter-status')?.addEventListener('change', e => { prodStatus = e.target.value; prodPage = 0; loadProductos(); });

const PROD_PER_PAGE = 20;

function toggleProdFiltrosAvanzados() {
  document.getElementById('prod-filtros-avanzados').classList.toggle('hidden');
}

function limpiarProdFiltrosAvanzados() {
  ['prod-f-model', 'prod-f-gb', 'prod-f-battery-min', 'prod-f-battery-max',
   'prod-f-price-min', 'prod-f-price-max', 'prod-f-color'].forEach(id => {
    document.getElementById(id).value = '';
  });
  document.getElementById('prod-f-battery-type').value = '';
  prodPage = 0;
  loadProductos();
}

async function loadProdFiltrosOpciones() {
  if (prodFiltrosOpciones) return; // solo una vez por sesión, los valores reales no cambian tan seguido
  const opciones = await api('/api/productos/filtros');
  if (!opciones) return;
  prodFiltrosOpciones = opciones;

  document.getElementById('prod-modelos-list').innerHTML =
    (opciones.modelos || []).map(m => `<option value="${m}">`).join('');
  document.getElementById('prod-colores-list').innerHTML =
    (opciones.colores || []).map(c => `<option value="${c}">`).join('');
  document.getElementById('prod-gb-list').innerHTML =
    (opciones.gb_disponibles || []).map(g => `<option value="${g}">`).join('');
}

async function loadProductos() {
  await loadProdFiltrosOpciones();

  const params = new URLSearchParams();
  if (prodSearch) params.set('search', prodSearch);
  if (prodStatus) params.set('status', prodStatus);

  const fModel = document.getElementById('prod-f-model')?.value;
  const fGb = document.getElementById('prod-f-gb')?.value;
  const fBatteryMin = document.getElementById('prod-f-battery-min')?.value;
  const fBatteryMax = document.getElementById('prod-f-battery-max')?.value;
  const fPriceMin = document.getElementById('prod-f-price-min')?.value;
  const fPriceMax = document.getElementById('prod-f-price-max')?.value;
  const fColor = document.getElementById('prod-f-color')?.value;
  const fBatteryType = document.getElementById('prod-f-battery-type')?.value;

  if (fModel) params.set('model', fModel);
  if (fGb) params.set('gb', fGb);
  if (fBatteryMin) params.set('battery_min', fBatteryMin);
  if (fBatteryMax) params.set('battery_max', fBatteryMax);
  if (fPriceMin) params.set('price_min', fPriceMin);
  if (fPriceMax) params.set('price_max', fPriceMax);
  if (fColor) params.set('color', fColor);
  if (fBatteryType) params.set('battery_type', fBatteryType);

  params.set('limit', PROD_PER_PAGE);
  params.set('offset', prodPage * PROD_PER_PAGE);
  ...
  document.getElementById('productos-table-body').innerHTML = data.productos.length
    ? data.productos.map(p => `
        <tr onclick="openProducto(${p.id})">
          ...
          <td>${p.color || '—'}</td>
          <td>${p.battery_pct ? p.battery_pct + '%' : '—'} ${p.battery_type === 'reemplazada' ? '<span class="badge" style="background:#f59e0b20;color:var(--yellow)" title="Batería cambiada por nosotros">Camb.</span>' : p.battery_type === 'cambiada_terceros' ? '<span class="badge" style="background:#ef444420;color:#ef4444" title="Batería cambiada por otro">Camb. 3°</span>' : ''}</td>
  ...
  const totalPages = Math.ceil(data.total / PROD_PER_PAGE);
  document.getElementById('productos-pagination').innerHTML = totalPages > 1
    ? Array.from({ length: totalPages }, (_, i) =>
        `<button class="${i === prodPage ? 'active' : ''}" onclick="goProductosPage(${i})">${i + 1}</button>`
      ).join('')
    : '';
}

function goProductosPage(p) { prodPage = p; loadProductos(); }
...
function batteryTypeLabel(t) {
  return { original: 'Original', reemplazada: 'Cambiada por nosotros', cambiada_terceros: 'Cambiada por otro' }[t] || t;
}
function equipmentTypeLabel(t) {
  return { usado: 'Usado', sellado: 'Sellado' }[t] || t;
}
  ...
  document.getElementById('modal-contact-body').innerHTML = `
    <div class="contact-detail">
      <div class="contact-meta-grid">
        ...
        <div class="contact-meta-item"><label>Garantía de batería</label><span>${p.warranty_months} meses</span></div>
        <div class="contact-meta-item"><label>Equipo</label><span>${p.equipment_type ? equipmentTypeLabel(p.equipment_type) : '—'}</span></div>
        <div class="contact-meta-item"><label>Garantía de equipo</label><span>${p.equipment_warranty_months ?? '—'} meses</span></div>
      </div>
...
// Autocompletar garantía de batería según su tipo (el usuario puede modificarla después)
...
// Autocompletar garantía de equipo — 3 meses por defecto en sellado o usado (el usuario puede modificarla después)
document.getElementById('new-prod-equipment-type')?.addEventListener('change', () => {
  document.getElementById('new-prod-equipment-warranty').value = 3;
});
...
async function loadSystemConfig() {
  ...
  form.cargador_solo.value = cfg.cargador_solo || '';
  form.funda.value = cfg.funda || '';
...
async function editProducto(id) {
  ...
  openEditModal('Editar producto', [
    ...
    { name: 'battery_type', label: 'Tipo de batería', value: p.battery_type || 'original', type: 'select', options: [
      ...
      { value: 'cambiada_terceros', label: 'Cambiada por otro' },
    ]},
    ...
    { name: 'warranty_months', label: 'Garantía de batería (meses)', value: p.warranty_months, type: 'number' },
    { name: 'equipment_type', label: 'Tipo de equipo', value: p.equipment_type || 'usado', type: 'select', options: [
      { value: 'usado', label: 'Usado' }, { value: 'sellado', label: 'Sellado' },
    ]},
    { name: 'equipment_warranty_months', label: 'Garantía de equipo (meses)', value: p.equipment_warranty_months ?? 3, type: 'number' },
  ...
  // Autocompletar garantía de batería si el usuario cambia el tipo (se puede editar después igual)
  ...
  // Autocompletar garantía de equipo si el usuario cambia el tipo (se puede editar después igual)
  const equipSelect = document.querySelector('#form-edit select[name="equipment_type"]');
  const equipWarrantyInput = document.querySelector('#form-edit input[name="equipment_warranty_months"]');
  if (equipSelect && equipWarrantyInput) {
    equipSelect.addEventListener('change', () => {
      equipWarrantyInput.value = 3;
    });
  }
}
...
document.getElementById('form-new-modelo')?.addEventListener('submit', async e => {
  ...
  if (btn.disabled) return; // ya hay una petición en curso, ignorar clicks de más
  btn.disabled = true;
  const d = Object.fromEntries(new FormData(e.target).entries());
  try {
